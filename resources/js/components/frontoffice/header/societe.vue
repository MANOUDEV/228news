...
const getResults = async () => {
  loading.value = true;
  await store.dispatch('societe/societeDataRequest', {});

  const status = store.getters['societe/getInfosSocieteStatus'];
  const message = store.getters['societe/getInfosSocieteMessage'];
  const data = store.getters['societe/getInfosSocieteData'];
        ...
        <div  v-if="empty === 1">
            <div class="row">
                ...
                <div class="col-md-6">
                  ...
                  <h5 style="text-align: center; margin-top: -50px"> {{ societeMessage  }} </h5>
                </div>
                <div class="col-md-3"></div>
            </div>
        </div>
