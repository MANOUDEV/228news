...
const politiqueData = ref({});
const politiqueMessage = ref(null);
const politiqueState = ref(true); 
const empty = ref(2);

const getResults = async () => { 
  await store.dispatch('politique/politiqueDataRequest', {});

  const status = store.getters['politique/getInfosPolitiqueStatus'];
  const message = store.getters['politique/getInfosPolitiqueMessage'];
  const data = store.getters['politique/getInfosPolitiqueData'];

  if (status === 'success') {
    politiqueData.value = data;
    empty.value = 0;
  } else {
    politiqueMessage.value = message;
...
<template>
    <div class="container">
      ...
      <div v-else class="row g-4 p-3 flex-fill">
            ...
            <div class="row">
                <div class="col-md-3"></div>
                    <div class="col-md-6">
                        ...
                        <h5 style="text-align: center; margin-top: -50px"> {{ politiqueMessage  }} </h5>
                    </div>
                <div class="col-md-3"></div>
            </div>
        </div>
        ...
        <div v-else-if="empty === 0" v-for="result in politiqueData" :key="result.id" class="col-sm-6 col-lg-3" >
